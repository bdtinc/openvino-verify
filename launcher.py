...
def show_platform_information() -> Dict[str, Any]:
    ...
    sysinfo["kernel_version"] = (
        os.uname().release if os.name == "posix" else platform.uname().release
    )
    ...
    for device in core.available_devices:
        sysinfo["available_devices"][device] = core.get_property(
            device, "FULL_DEVICE_NAME"
        )
...
def print_result_line(
    tests: Dict[str, str], test_file: str, test_results: Dict[str, Any]
) -> None:
    RED = "\033[91m"
    GREEN = "\033[92m"
    CLOSE = "\033[0m"
...
def run_tests(
    tests: Dict[str, str], test_dir: str, device: Optional[str] = None
...
https://github.com/openvinotoolkit/openvino_contrib/tree/master/modules/nvidia_plugin"""
        ...
        print("")
        print(
            f"Full error messages in ERRORS.txt and Errors section of {results_json_filename}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tests_dir", help="Directory containing test files", default=TEST_DIR
    )
    ...
    parser.add_argument(
        "--run_files",
        nargs="+",
        help="List of test filenames to run. Files are relative to TESTS_DIR.",
    )
    parser.add_argument(
        "--tests_json",
        help="JSON file containing tests to run in the format {filename: test description}",
    )
